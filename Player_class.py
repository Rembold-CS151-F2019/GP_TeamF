...
class Player:
    ...
    def control(self, key):
        ...
        moves = 0
        if key == "Up" and m.Maze.check_wall != "wall":
            moves += 1
            self.object.move(0, -5)
        if key == "Down" and m.Maze.check_wall != "wall":
            moves += 1
            self.object.move(0, 5)
        if key == "Left" and m.Maze.check_wall != "wall":
            moves += 1
            self.object.move(-5, 0)
        if key == "Right" and m.Maze.check_wall != "wall":
            moves += 1
            self.object.move(5, 0)